...
3. 사용자로부터 a, b, c에 해당되는 세 개의 int형 정수를 입력 받고 $ax^2 + bx + c = 0$ 이라는 이차방정식의 해를 구해서 출력하는 프로그램을 작성한다. 해를 구하는 것은 근의 공식을 이용한다. $b^2 - 4 \times a \times c >= 0$ 조건을 만족하는 정수 a, b, c 값을 사용자가 입력한다고 가정하고, 제곱근은 Math.sqrt() 함수를 사용해서 구한다.
```java
import java.util.Scanner;
...
public class Prac3 {
    public static void main(String[] args) {
        ...
        System.out.printf("이차 방정식의 해는 %.2f,%.2f 입니다.\n", x1, x2);
    }
}
```

4. 사용자로부터 R1, R2, Height 값을 double형으로 입력 받고, R1을 윗면, R2를 밑면, Height를 높이로 하는 원뿔대의 부피를 구해서 출력하는 프로그램을 작성한다. 원주율 값은 3.14로 계산한다. ($V = \frac{1}{3} \times \pi \times (R1^2 + R1R2 + R2^2) \times Height $)
```java
import java.util.Scanner;
...
public class Prac4 {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        System.out.println("윗면, 밑면, 높이를 입력해주세요: ");
        ...
        double pi = 3.14;
        double v;
        v = 1.0 / 3.0 * pi * (r1 * r1 + r1 * r2 + r2 * r2) * height;

        System.out.println("원뿔대의 부피는 " + v + " 입니다.");
    }
}
```

5. 사용자로부터 온도(화씨)와 풍속(m/s)을 입력 받고, 체감 온도를 계산하고 화면에 출력하는 프로그램을 작성한다.
...
public class Prac5 {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        System.out.println("온도와 풍속을 입력해주세요: ");

        double t = sc.nextDouble();
        double v = sc.nextDouble();

        double st;
        st = 13.12 + 0.6215 * t - 11.37 * Math.pow(v, 0.16) + 0.3965 * Math.pow(v, 0.16) * t;

        System.out.println("체감온도는 " + st + "도 입니다.");
    }
}
```

6. 사용자로부터 문자 5개를 입력 받고 배열에 저장한 후에 해시 값을 구해서 화면에 출력하는 프로그램을 작성한다. 해시 값은 소수 둘째 자리까지 출력한다. 만약 문자들을 저장한 배열이 s라고 가정할 때 아래 공식에 따라 해시 값을 계산한다. 첫 번째로 입력한 문자가 s[0], 두 번째는 s[1]에, 그리고 나머지 문자들도 순서대로 저장된다고 가정한다. 승수 계산은 Math.pow(x, y)함수를 사용한다.
...
public class Prac6 {
    public static void main(String[] args) {
        ...
        hashValue = s[0] * Math.pow(31, 4) + s[1] * Math.pow(31, 3) + s[2] * Math.pow(31, 2) + s[1] * Math.pow(31, 1) + s[0] * Math.pow(31, 0);

        System.out.printf("해시 값은 %.2f입니다.\n", hashValue);
    }
}
```

7. 사용자에게서 5개 정수를 입력 받을 때까지 프로그램을 실행시키면서, 매 숫자 입력 후에 그때까지 입력된 모든 정수값들의 합을 계산해서 출력하는 프로그램을 작성한다.(반복문 사용 없이)
```java
import java.util.Scanner;
...
8. 사용자로부터 두 개의 double형 실수를 입력 받고, 두 수의 덧셈, 뺄셈, 곱셈, 나눗셈 연산 결과를 출력하는 프로그램을 작성하라.
```java
import java.util.Scanner;
...
public class Prac8 {
    public static void main(String[] args) {
        ...
        System.out.println("두 개의 실수를 입력해주세요: ");
        double d1 = sc.nextDouble();
        double d2 = sc.nextDouble();

        System.out.printf("두 수의 덧셈 결과: %f\n", d1 + d2);
        System.out.printf("두 수의 뺄셈 결과: %f\n", d1 - d2);
        System.out.printf("두 수의 곱셈 결과: %f\n", d1 * d2);
        System.out.printf("두 수의 나눗셈 결과: %f\n", d1 / d2);

    }
}
```

9. 배열을 1, 3, 5, 7, 9로 초기화하고 배열 요소의 합과 평균을 구하는 프로그램을 작성한다.(반복문 사용 없이)
```java
public class Prac9 {
    public static void main(String[] args) {
        int[] arr = {1, 3, 5, 7, 9};
...
10. 배열을 문자열 "one", "two", "three"로 초기화시키고, 각 문자열을 화면에 순서대로 출력하는 프로그램을 작성한다.(반복문 사용 없이)
```java
public class Prac10 {
    public static void main(String[] args) {
...
11. 다음 표는 특정 시기 서울의 미세먼지 현황을 보인다. 7일 동안의 PM2.5와 PM10의 평균을 계산해서 화면에 출력한다. 단 PM2.5와 PM10 데이터를 각각 배열에 초기화시킨 후에 평균을 구한다.(반복문 사용 없이)
...
public class Prac11 {
    public static void main(String[] args) {
        int[] pm2_5 = {29, 21, 18, 34, 67, 79, 18};
        int[] pm10 = {44, 44, 32, 60, 84, 112, 41};

        int sum2_5 = pm2_5[0] + pm2_5[1] + pm2_5[2] + pm2_5[3] + pm2_5[4] + pm2_5[5] + pm2_5[6];
        int sum10 = pm10[0] + pm10[1] + pm10[2] + pm10[3] + pm10[4] + pm10[5] + pm10[6];

        double avg2_5 = sum2_5 / 7.0;
        double avg10 = sum10 / 7.0;

        System.out.printf("PM2.5의 평균: %f, PM10의 평균: %f\n", avg2_5, avg10);

    }
}
```

12. 사용자로부터 (x1, y1), (x2, y2)에 해당되는 정보를 double형으로 입력 받고 두 점 사이의 거리를 계산해서 출력하는 프로그램을 작성한다. 거리는 두 점 사이의 거리를 계산하는 공식을 이용한다.
...
public class Prac12 {
    public static void main(String[] args) {
        ...
        System.out.println("첫 번째 좌표를 입력해주세요: ");
        ...
        System.out.printf("두 점사이의 거리: %f\n", distance);
    }
}
```

13. 헤론의 공식은 세 변의 길이만으로 삼각형의 넓이를 구하는 공식이다. 세변의 길이를 각각 a, b, c라고 할 때 사용자로부터 세 변의 길이를 입력 받고 삼각형의 면적을 계산해서 출력하는 프로그램을 작성한다.
...
public class Prac13 {
    public static void main(String[] args) {
        ...
        System.out.println("세 변의 길이를 입력해주세요(a b c): ");
...
14. 사용자로부터 삼각형을 이루는 세 점의 좌표를 입력 받고 면적을 구해서 출력하는 프로그램을 작성한다. 좌표는 정수로 입력 받는다.
```java
import java.util.Scanner;
...
public class Prac14 {
    public static void main(String[] args) {
        ...
        System.out.println("첫 번째 점의 좌표를 입력해주세요:");
        int x1 = sc.nextInt();
        int y1 = sc.nextInt();

        System.out.println("두 번째 점의 좌표를 입력해주세요:");
        int x2 = sc.nextInt();
        int y2 = sc.nextInt();

        System.out.println("세 번째 점의 좌표를 입력해주세요:");
        int x3 = sc.nextInt();
        int y3 = sc.nextInt();
...
15. 한 달에 x원씩 연 이율 r%인 적금을 넣는다고 가정해보자. 이자 소득세율이 t%라고 가정할 때 1년 만기 후 내가 받을 수 있는 금액이 얼마인지 계산해서 출력하는 프로그램을 작성한다. 불입한 원금에 대한 이자만 계산하는 단리 계산 방식은 다음 공식에 따른다. 이자에 대한 소득세는 전체 이자 금액에 대해서 일정 비율 차감한다.
...
public class Prac15 {
    public static void main(String[] args) {
        ...
        System.out.println("한 달에 넣는 납입액을 입력해주세요: ");
        int monthlyPayment = sc.nextInt();

        System.out.println("연 이율을 입력해주세요: ");
        double r = sc.nextDouble();

        System.out.println("이자 소득 세율을 입력해주세요: ");
        double t = sc.nextDouble();

        int n = 12;

        double interest;
        interest = monthlyPayment * n * ((n + 1) / 2.0) * (r / 12.0);
        interest = interest * (1 - t / 100);
        double total = monthlyPayment * n + interest;

        System.out.println("총 받을 수 있는 금액은 " + total +"원 입니다.");
...
public class Prac2 {
    public static void main(String[] args) {
        ...
        System.out.println("치환할 문자열을 입력하세요: ");
        String target = sc.nextLine();

        System.out.println("새로운 문자열을 입력하세요: ");
        String replace = sc.nextLine();

        String newStr = original.replace(target, replace);
...
3. 여러 카드 중에서 한 장의 카드를 나타내는 클래스를 구현한다. 카드 클래스의 show() 멤버 함수를 호출하면 카드에 있는 모양과 번호를 화면에 출력하도록 한다.(하트 10, J, Q, K, A)
```java
import java.util.Random;

class Card{
    private String shape;
    private String[] numbers = {"10", "J", "Q", "K", "A"};
...
class StringMirror{
    ...
    public static void main(String[] args){
        StringMirror stringMirror = new StringMirror();
        System.out.println(stringMirror.mirror("Sangmyung University"));
    }
...
public class Prac5 {
    public static void main(String[] args) {
        String[] cities = {"Seoul", "Incheon", "Seongnam"};
        String front = "www.";
        String end = ".go.kr";

        System.out.println(front + cities[0] + end);
        System.out.println(front + cities[1] + end);
        System.out.println(front + cities[2] + end);
    }
}
```
...
public class Prac7 {
    public static void main(String[] args) {
        ...
        System.out.println("두 글자 이상의 문자열을 입력하세요: ");
        String str = sc.nextLine();
        int sum = 0;
        sum += (str.charAt(0) + str.charAt(1));
        int index = sum % 10;
        sarr[index] = str;
        ...
        System.out.println(sarr[6]);
        System.out.println(sarr[7]);
        System.out.println(sarr[8]);
        System.out.println(sarr[9]);
...
8. 사용자로부터 문자열 한 개를 입력 받고, 단어의 개수를 센 뒤, 문자열의 길이, 단어의 개수를 화면에 출력하는 프로그램을 작성한다. 단어 구별은 공백 문자(' ','\t', '\n')로 한다. 공백 문자는 두 개 이상 연속적으로 나타날 수 있다.
```java
import java.util.Scanner;
...
public class Prac8 {
    public static void main(String[] args) {
        ...
        System.out.println("문자열 한 개를 입력해주세요: ");
        String str = sc.nextLine();
        str.trim().replaceAll("\\s+", " ");
        String[] words;
        words = str.split(" ");

        System.out.println("문자열의 길이: "+ str.length() + ", 단어의 개수: " + words.length);
    }
}
```
...
public class Prac9 {
    public static void main(String[] args) {
        ...
        System.out.println("새로운 확장자를 입력해주세요: ");
...
public class Prac1 {
    public static void main(String[] args) {
        ...
        if(year <= 0){
            System.out.println("제대로 된 년도가 아닙니다.");
        }
        else{
...
public class Prac3 {
    public static void main(String[] args) {
        ...
        System.out.print("A 상점까지의 거리(km): ");
        double distanceA = sc.nextDouble();
        System.out.print("B 상점까지의 거리(km): ");
        double distanceB = sc.nextDouble();

        System.out.print("A상점의 컴퓨터 가격(원): ");
        int priceA = sc.nextInt();
        System.out.print("B상점의 컴퓨터 가격(원): ");
        int priceB = sc.nextInt();

        //유류비 및 시간
        double fuelA = (distanceA / 10) * 1400;
        double timeA = (distanceA / 40) * 10000;

        double fuelB = (distanceB / 10) * 1400;
        double timeB = (distanceB / 40) * 10000;

        //총 비용
        double totalA = priceA + fuelA + timeA;
        double totalB = priceB + fuelB + timeB;

        if(totalA < totalB){
            System.out.printf("더 싼 상점: A, 가격: %f\n", totalA);
        }
        else{
            System.out.printf("더 싼 상점: B, 가격: %.2f원\n", totalB);
        }
    }
}
...
class CardGame{
    public static void play(){
        ...
        System.out.println("첫번째 카드: " + cardNames[index1]);
        System.out.println("두번째 카드: " + cardNames[index2]);

        if(index1 < index2){
            System.out.println("두번째 카드가 더 높은 값입니다.");
        }
        else if(index1 > index2){
            System.out.println("첫번째 카드가 높은 값입니다.");
        }
        else{
            System.out.println("두 카드의 값이 같습니다.");
        }
    }
...
public class Prac5 {
    public static void main(String[] args) {
        ...
        do{
            System.out.println("2 이상의 정수 한 개를 입력해주세요: ");
            int n = sc.nextInt();

            if(n < 2){
                System.out.println("잘못 입력했습니다. 2 이상의 정수를 입력하세요");
            }
            else{
                for(int i = 1; i <= n; i++){
                    if(n % i == 0){
...
public class Prac6 {
    public static void main(String[] args) {
        ...
        System.out.print("2 이상의 정수 한 개를 입력해주세요: ");
        int n = sc.nextInt();
...
public class Prac7 {
    public static void main(String[] args) {
        ...
        if(divcount == 2){
            System.out.printf("정수: %d  약수 개수: %d  소수: true\n", n, divcount);
        }
        else{
            System.out.printf("정수: %d  약수 개수: %d  소수: false\n", n, divcount);
        }
    }
}
...
public class Prac8 {
    public static void main(String[] args) {
        ...
        System.out.print("월을 영어로 입력해주세요: ");
        String month = sc.next();

        for(int i = 0; i < 12; i++){
            if(month.equals(months[i])){
                System.out.println(month + "월에는 " + days[i] + "일까지 있습니다.");
            }
        }
    }
}
...
public class Prac12 {
    public static void main(String[] args) {
        ...
        System.out.println("1~10 사이의 숫자: " + section1 + "개\n11~20 사이의 숫자: " + section2 + "개\n21~30 사이의 숫자: " + section3 + "개");
...
public class Prac13 {
    public static void main(String[] args) {
        for(int n = 1000; n <= 9999; n++){
            ...
            if(n == Math.pow(n1, 4) + Math.pow(n2, 4) + Math.pow(n3, 4) + Math.pow(n4, 4)){
...
public class Prac14 {
    public static void main(String[] args) {
        ...
        String str = "Lorem ipsum dolor sit amet, consectetur adipiscing elit. " +
                    "Curabitur nunc tellus, faucibus quis maximus in, sodales in est. " +
                    "Aenean nec ipsum mauris. " +
                    "Morbi bibendum bibendum dui, commodo convallis turpis volutpat eu. " +
                    "Etiam consectetur a magna pretium mollis. Duis egestas ultrices lacinia. " +
                    "Morbi nunc sapien, varius sit amet maximus id, pharetra eget ipsum. " +
                    "Quisque interdum augue mauris, non facilisis ligula consectetur sit amet. " +
                    "Nam scelerisque est sed convallis sodales. " +
                    "Proin tempor nibh sit amet neque pretium, nec elementum libero mattis. " +
                    "Duis commodo eu nisl non varius. Mauris dapibus quam eros, nec volutpat magna tincidunt fermentum. " +
                    "Cras elit leo, porta quis pulvinar a, condimentum rutrum justo. " +
                    "Nunc et odio fringilla sem rhoncus efficitur sit amet vitae ligula. " +
                    "Pellentesque vel aliquam magna, vitae auctor metus.";
...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.print("주사위를 던질 횟수를 입력해주세요: ");
        int n = sc.nextInt();
...
class Scores{
    ...
    public void addScore(int score){
        if(count < scores.length){
            ...
            System.out.println("점수를 더이상 추가할 수 없습니다.");
        }
    }
...
public class Prac17 {
    public static void main(String[] args) {
        Scores englishScores = new Scores(SubjectType.ENGLISH);
        Scores mathScores = new Scores(SubjectType.MATH);

        englishScores.addScore(80);
        englishScores.addScore(75);
        ...
        System.out.println("영어 평균: " + englishScores.getAverage());
        System.out.println("수학 평균: " + mathScores.getAverage());
...
enum CardType{
    ...
    CardType(int discountRate, int accumulationRate){
...
public class Prac18 {
    public static void main(String[] args) {
        ...
        if(cardType != null){
            ...
            System.out.println("해당하는 카드가 없습니다.");
        }
    }

    //입력 받은 카드 이름에 해당하는 enum 값 반환
    private static CardType getCardType(String cardName){
        for(CardType cardType : CardType.values()){
            if(cardType.name().equalsIgnoreCase(cardName.replace(" ", "_"))){
                return cardType;
            }
        }
        return null;
    ...
    //카드 정보 출력
    private static void printCard(CardType cardType){
        System.out.println("할인율: " + cardType.getDiscountRate() + "%");
        System.out.println("적립률: " + cardType.getAccumulationRate() + "%");
    }
}
...
public class Prac1 {
    public static void main(String[] args){
        ...
        for(int i = 0; i < 100; i++){
            array[i] = (int)(Math.random() * 6) + 5;
        }
        System.out.println(countNum(array, 7));
    }
...
public class Prac2 {
    public static void main(String[] args){
        String s = "Aenean fringilla tincidunt sapien, a luctus ligula tempor.";

        System.out.println(countChar(s, 'n'));
    }
...
public class Prac {
    public static void main(String[] args){
        ...
        String s = "sangmyung university";
        System.out.println(mirrorStr(s));
    }

    private static String mirrorStr(String str){
...
public class Prac4 {
    ...
    // 생일을 입력받아 별자리를 반환하는 함수
    public static ZodiacSign findZodiacSign(int month, int day) {
        switch (month) {
            case 1:
    ...
    public static void main(String[] args) {
        ...
        // 별자리 찾기
        ZodiacSign zodiacSign = findZodiacSign(month, day);

        // 출력
        System.out.println("당신의 별자리는 " + zodiacSign.getSignName() + "입니다.");
    }
}
...
public class Prac5 {
    public enum ZodiacSign {
        ARIES("양자리", "2023-03-21", "2023-04-19"),
        TAURUS("황소자리", "2023-04-20", "2023-05-20"),
        GEMINI("쌍둥이자리", "2023-05-21", "2023-06-20"),
        CANCER("게자리", "2023-06-21", "2023-07-22"),
        LEO("사자자리", "2023-07-23", "2023-08-22"),
        VIRGO("처녀자리", "2023-08-23", "2023-09-22"),
        LIBRA("천칭자리", "2023-09-23", "2023-10-22"),
        SCORPIO("전갈자리", "2023-10-23", "2023-11-21"),
        SAGITTARIUS("사수자리", "2023-11-22", "2023-12-21"),
        CAPRICORN("염소자리", "2023-12-22", "2024-01-19"),
        AQUARIUS("물병자리", "2024-01-20", "2024-02-18"),
        PISCES("물고기자리", "2024-02-19", "2024-03-20");

        private final String name;
        private final Date startDate;
        private final Date endDate;

        ZodiacSign(String name, String startDate, String endDate) {
            this.name = name;
    ...
    public static void main(String[] args) {
        ...
        System.out.print("별자리 이름을 입력하세요: ");
        String userInput = scanner.nextLine();

        // 입력 받은 별자리에 대한 기간 출력
        ZodiacSign zodiacSign = getZodiacSign(userInput);
        if (zodiacSign != null) {
            System.out.println(zodiacSign.getName() + "의 기간: " +
                    new SimpleDateFormat("yyyy-MM-dd").format(zodiacSign.getStartDate()) + " ~ " +
                    new SimpleDateFormat("yyyy-MM-dd").format(zodiacSign.getEndDate()));
        } else {
            System.out.println("해당하는 별자리가 없습니다.");
        }
    }

    private static ZodiacSign getZodiacSign(String userInput) {
        for (ZodiacSign sign : ZodiacSign.values()) {
            if (sign.getName().equals(userInput)) {
                return sign;
            }
        }
        return null;
...
public class Prac6 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Choose data type: 1. int  2. float  3. double");
...
7. 정수 한 개를 입력 받고 1과 자기 자신을 제외한 약수의 개수를 반환하는 함수를 작성한다. 이 함수를 이용해서 2 ~ 99까지의 약수 개수를 출력하는 프로그램을 작성한다. 단 약수가 없는 소수인 경우에는 소수라고 출력한다.
```java
public class Prac7 {
    ...
    static int countDivisors(int n) {
        ...
        return count - 2; // 1과 n 자체를 뺀 약수의 개수
    }
    ...
    public static void main(String[] args) {
        for (int i = 2; i <= 99; ++i) {
            ...
            if (divisorsCount == 0) {
                ...
                System.out.println(i + "의 약수 개수: " + divisorsCount);
            }
        }
    }
}
...
public class Prac8 {
    public static void main(String[] args) {
        ...
        // 각 문자열의 해시 값을 계산하고 출력
        for (int i = 0; i < 5; i++) {
            double hashValue = calculateHash(strings[i]);
            System.out.println("문자열: " + strings[i] + ", 해시 값: " + hashValue);
        }
    }
...
public class Prac10 {
    ...
    // 생성자
    public ResidentRegistrationNumber(String registrationNumber) {
    ...
    public boolean isValid() {
        ...
        // 생년월일 부분이 숫자로 이루어져 있는지 확인
        String birthDate = registrationNumber.substring(0, 6);
        ...
        // '-' 기호 확인
        if (registrationNumber.charAt(6) != '-') {
            return false;
        }

        // 뒷부분이 숫자로 이루어져 있는지 확인
        String tail = registrationNumber.substring(7);
        for (char ch : tail.toCharArray()) {
            if (!Character.isDigit(ch)) {
                return false;
            }
        }

        // 모든 조건을 만족하면 유효한 주민등록번호로 판단
        return true;
    }
    ...
    public static void main(String[] args) {
        // 테스트
        ResidentRegistrationNumber rrn = new ResidentRegistrationNumber("123456-7890123");

        if (rrn.isValid()) {
            System.out.println("유효한 주민등록번호입니다.");
            System.out.println("생년월일의 월: " + rrn.getMonth());
        } else {
...
public class Prac11 {
    ...
    public static void main(String[] args) {
        ...
        // 결과 출력
        System.out.println("입력한 숫자들의 표준 편차: " + stdDeviation);
...
public class Prac12 {
    ...
    /**
     * 사용자로부터 주어진 범위(low~high, high 포함)의 정수를 입력 받아서 변환.
     * 범위 밖의 정수가 입력되면 제대로 입력할 때까지 반복해서 입력 받음.
     */
    public int readIntInRange(int low, int high) {
        Scanner scanner = new Scanner(System.in);
        int input;

        do {
            System.out.println("정수를 입력하세요 (" + low + "~" + high + "): ");
     ...
     */
    public int getDigit(int placeNumber) {
        int digit;

        if (placeNumber == 0) {
            digit = this.userInput % 10; // 일의 자리 숫자 추출
        } else {
            digit = (this.userInput / placeNumber) % 10; // 다른 자리 숫자 추출
        }
    ...
    public static void main(String[] args) {
        ...
        int input = digitExtractor.readIntInRange(1000, 9999);

        System.out.println("천의 자리 숫자: " + digitExtractor.getDigit(1000));
        System.out.println("백의 자리 숫자: " + digitExtractor.getDigit(100));
        System.out.println("십의 자리 숫자: " + digitExtractor.getDigit(10));
        System.out.println("일의 자리 숫자: " + digitExtractor.getDigit(1));
    }
}
...
public class Prac13 {
    ...
    public static void main(String[] args) {
        ...
        // getIntArray 함수를 이용하여 배열 구성
        int[] numbers = getIntArray(size);

        // 배열의 평균 계산
        double average = calculateAverage(numbers);

        // 배열의 각 요소와 평균 출력
        System.out.println("배열의 각 요소와 평균:");
        for (int i = 0; i < size; i++) {
            System.out.println("요소 " + (i + 1) + ": " + numbers[i] + ", 평균: " + average);
        }
    }
}
...
public class Prac14 {
    ...
    public static void printSumMinMax() {
        ...
        // 30개의 무작위 정수 생성 및 합계, 최대값, 최소값 계산
        for (int i = 0; i < 30; i++) {
            int randomNumber = random.nextInt(100); // 0부터 99까지의 난수 생성 (임의로 범위 지정)
...
public class Prac15 {
    public enum ViolationCase {
        LIGHT, PEDESTRIAN, YELLOW, BUSONLYHIGHWAY, BUSONLY, LANE, OVER60, OVER4060, OVER2040, OVER20
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("법규 위반 사례를 입력하세요: ");
        String violationCaseInput = scanner.nextLine();
        ViolationCase violationCase = ViolationCase.valueOf(violationCaseInput.toUpperCase());

        System.out.println("차종을 입력하세요 (VAN, CAR, AUTO 중 하나): ");
        String carTypeInput = scanner.nextLine();
        CarType carType = CarType.valueOf(carTypeInput.toUpperCase());

        int fine = calculateFine(violationCase, carType);

        if (fine == -1) {
            System.out.println("올바르지 않은 입력입니다.");
        } else {
            System.out.println("교통 범칙금은 " + fine + "만원 입니다.");
        }

        scanner.close();
    }

    public static int calculateFine(ViolationCase violationCase, CarType carType) {
        switch (violationCase) {
            case LIGHT:
                return getFineAmount(carType, 8, 7, 5);
            case PEDESTRIAN:
                return getFineAmount(carType, 8, 7, 5);
            case YELLOW:
                return getFineAmount(carType, 10, 9, 0);
            case BUSONLYHIGHWAY:
                return getFineAmount(carType, 10, 9, 0);
            case BUSONLY:
                return getFineAmount(carType, 0, 5, 4);
            case LANE:
                return getFineAmount(carType, 4, 4, 3);
            case OVER60:
                return getFineAmount(carType, 14, 13, 9);
            case OVER4060:
                return getFineAmount(carType, 11, 10, 7);
            case OVER2040:
                return getFineAmount(carType, 8, 7, 5);
            case OVER20:
                return getFineAmount(carType, 4, 4, 3);
    ...
    public static int getFineAmount(CarType carType, int vanFine, int carFine, int autoFine) {
        switch (carType) {
            case VAN:
                return vanFine;
            case CAR:
...
public class Prac16 {
    public static void main(String[] args) {
        ...
        // 반복문을 사용하여 1부터 N까지 출력
        System.out.println("반복문을 사용한 출력:");
        for (int i = 1; i <= N; i++) {
            System.out.print(i + " ");
        }
...
public class PrintNumbers {
    public static void main(String[] args) {
        ...
        // 재귀 호출을 사용하여 1부터 N까지 출력
        System.out.println("재귀 호출을 사용한 출력:");
...
```java
public class Prac17 {
    public static int findGCDIterative(int m, int n) {
        while (n != 0) {
            int temp = m;
            m = n;
            n = temp % n;
    ...
    public static void main(String[] args) {
        int m = 48;
        int n = 18;
        int result = findGCDIterative(m, n);
...
```java
public class Prac17 {
    public static int findGCDRecursive(int m, int n) {
        if (n == 0) {
            return m;
        } else {
            return findGCDRecursive(n, m % n);
        }
    }
    ...
    public static void main(String[] args) {
        int m = 48;
        int n = 18;
        int result = findGCDRecursive(m, n);
        System.out.println("최대 공약수: " + result);
    }
}
...
class Customer {
    ...
    public Customer(int customerId, String name, String phoneNumber, String address, int mileage) {
...
class Order {
    ...
    public Order(int orderId, String productName, int quantity, double amount, String time, String status, String shippingAddress) {
...
class Prac1 {
    public static void main(String[] args) {
        Customer customer = new Customer(1, "John Doe", "123-456-7890", "123 Main St", 100);

        Order order1 = new Order(101, "Product A", 2, 50.0, "2023-01-01 10:00", "Shipped", "456 Side St");
        Order order2 = new Order(102, "Product B", 1, 30.0, "2023-02-02 15:30", "Processing", "789 Back St");

        customer.setMileage(customer.getMileage() + (int) (order1.getAmount() + order2.getAmount()));
        ...
        System.out.println("Mileage: " + customer.getMileage());
...
class Car {
    ...
    public Car(int id, String manufacturer, String modelName, int manufacturingYear, double price, double mileage) {
        this.id = id;
...
class Dealer {
    ...
    public void removeCar(int carId) {
        for (Car car : availableCars) {
            if (car.getId() == carId) {
                ...
                System.out.println("Car removed from the inventory.");
                return;
            }
        }
    ...
    public ArrayList<Car> searchCarById(int carId) {
    ...
    public ArrayList<Car> searchCarByPriceRange(double minPrice, double maxPrice) {
        ArrayList<Car> result = new ArrayList<>();
        for (Car car : availableCars) {
...
public class Prac2 {
    public static void main(String[] args) {
        ...
        myDealer.addCar(new Car(1, "Toyota", "Camry", 2020, 25000.0, 30000.0));
        myDealer.addCar(new Car(2, "Honda", "Accord", 2019, 22000.0, 35000.0));
        myDealer.addCar(new Car(3, "Ford", "Fusion", 2018, 20000.0, 40000.0));

        int searchCarId = 2;
        ArrayList<Car> resultById = myDealer.searchCarById(searchCarId);
        if (!resultById.isEmpty()) {
            System.out.println("Car with ID " + searchCarId + " found: " + resultById.get(0).getModelName());
        } else {
            System.out.println("Car with ID " + searchCarId + " not found.");
        }

        double minPrice = 22000.0;
        double maxPrice = 25000.0;
        ArrayList<Car> resultByPriceRange = myDealer.searchCarByPriceRange(minPrice, maxPrice);
        if (!resultByPriceRange.isEmpty()) {
            System.out.println("Cars in the price range " + minPrice + " - " + maxPrice + ":");
            for (Car car : resultByPriceRange) {
...
// DocConverter 클래스: 파일을 다른 포맷으로 변환하는 모듈
class DocConverter {
    public void convert(String document, String targetFormat) {
        // 문서 변환 로직을 구현
        System.out.println("Converting document to " + targetFormat + " format: " + document);
    }
}

// WordProcessor 클래스: SpellChecker와 DocConverter를 포함하는 워드프로세서 클래스
class WordProcessor {
    private SpellChecker spellChecker;
    private DocConverter docConverter;

    public WordProcessor(SpellChecker spellChecker, DocConverter docConverter) {
    ...
    public void processDocument(String document) {
        ...
        // 문서 저장 (기본적으로 현재 확장자를 유지하며 저장)
        docConverter.convert(document, "same_format");
    }
}

public class Prac3 {
    public static void main(String[] args) {
        // SpellChecker 및 DocConverter 인스턴스 생성
        SpellChecker spellChecker = new SpellChecker();
        DocConverter docConverter = new DocConverter();

        // WordProcessor 인스턴스 생성 및 초기화
        WordProcessor wordProcessor = new WordProcessor(spellChecker, docConverter);

        // 예시 문서
        String document = "This is a sample document.";

        // WordProcessor를 사용하여 문서 처리
        wordProcessor.processDocument(document);
    }
}
...
enum CardType {
    ...
    SHM_LADY(25, 2),
    KE_STAR(20, 1),
    EMT_KE_OUTLAND(15, 0); // Enum for the same card in Outland
...
public class Prac4 {
    public static void main(String[] args) {
        ...
        System.out.print("레스토랑을 선택하세요 (VANS 또는 OUTLAND): ");
        String restaurantInput = scanner.nextLine().toUpperCase();

        try {
            Restaurant selectedRestaurant = Restaurant.valueOf(restaurantInput);
            ...
            switch (selectedRestaurant) {
                case VANS:
                    cardType = CardType.valueOf(cardInput);
                    break;
                case OUTLAND:
                    // Check if the card is available in both VANS and OUTLAND
                    cardType = CardType.valueOf(cardInput + "_OUTLAND");
                    break;
            }
            ...
            if (cardType != null) {
                ...
                System.out.println("잘못된 레스토랑 또는 카드 이름입니다.");
            }

        } catch (IllegalArgumentException e) {
            System.out.println("잘못된 레스토랑을 선택하셨습니다.");
        } finally {
            scanner.close();
        }
    }
...
class AddressBook {
    ...
    public AddressBook() {
        this.addressList = new Address[MAX_ADDRESSES];
        this.size = 0;
    }

    public void addAddress(Address address) {
        if (size < MAX_ADDRESSES) {
            addressList[size++] = address;
            System.out.println("Address added successfully.");
        } else {
            System.out.println("Address book is full. Cannot add more addresses.");
        }
    }
    ...
    public void searchAddress(String keyword) {
        boolean found = false;
        for (int i = 0; i < size; i++) {
            if (addressList[i].getName().contains(keyword) ||
                addressList[i].getPhoneNumber().contains(keyword) ||
                addressList[i].getAddress().contains(keyword)) {
                System.out.println(addressList[i]);
                found = true;
            }
        }
...
public class AddressBookProgram {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            System.out.println("2. Display All Addresses");
            ...
            switch (choice) {
                case 1:
                    ...
                    Address newAddress = new Address(name, phoneNumber, addressString);
                    addressBook.addAddress(newAddress);
                    break;
                case 2:
                    addressBook.displayAllAddresses();
                    break;
                case 3:
                    System.out.print("Enter a keyword to search: ");
                    String keyword = scanner.nextLine();
                    addressBook.searchAddress(keyword);
                    break;
                case 4:
...
class Sedan implements Vehicle {
    boolean signalOn; // 시동 여부를 나타내는 변수
    int speed; // 속도를 나타내는 변수

    // 생성자
    public Sedan() {
        this.signalOn = false; // 초기에 시동이 꺼져있음
        this.speed = 0; // 초기 속도는 0
    }

    // Vehicle 인터페이스의 메소드 구현
    @Override
    public void signalTurn(Direction direction, boolean signalOn) {
        // 여기에 필요한 동작을 구현
        // 예를 들어, 좌우 방향으로 신호등을 켜거나 끄는 동작 등
    }
    ...
    public void changeSpeed(int newSpeed) {
        // 속도 변경 메소드 구현
        this.speed = newSpeed;
        System.out.println("changing speed now at " + newSpeed);
    }
    ...
    public void start() {
        ...
        this.signalOn = true; // 시동이 걸린 상태로 변경
    }
    ...
    public void stop() {
        ...
        this.signalOn = false; // 시동이 꺼진 상태로 변경
    }
}
...
public class Prac1 {
    public static void main(String[] args) {
        ...
        // 속도 변경
        mySedan.changeSpeed(10);

        // 속도 변경
        mySedan.changeSpeed(20);

        // 속도 변경
        mySedan.changeSpeed(30);
...
public class Shape {
    public double calculateArea() {
        return 0.0;  // 기본값, 하위 클래스에서 오버라이드될 것임
    }
...
// 저장 장치 클래스
class StorageDevice implements Connection {
    protected String modelName;
    protected int capacity;

    public StorageDevice(String modelName, int capacity) {
        this.modelName = modelName;
    ...
    @Override
    public String toString() {
        return "Model: " + modelName + ", Capacity: " + capacity + "GB";
    }
}
...
class HDD extends StorageDevice {
    ...
    public HDD(String modelName, int capacity, int rpm, double size) {
        super(modelName, capacity);
        this.rpm = rpm;
        this.size = size;
    }
    ...
    @Override
    public String toString() {
        return super.toString() + ", RPM: " + rpm + ", Size: " + size;
    }
}

// 외장형 HDD 클래스
class ExternalHDD extends HDD {
    public ExternalHDD(String modelName, int capacity, int rpm, double size) {
        super(modelName, capacity, rpm, size);
    }

    @Override
    public void connected() {
        System.out.println("USB로 연결됨");
    }
}

// 내장형 HDD 클래스
class InternalHDD extends HDD {
    public InternalHDD(String modelName, int capacity, int rpm, double size) {
        super(modelName, capacity, rpm, size);
    }

    @Override
    public void connected() {
        System.out.println("SATA로 연결됨");
    }
}

// USB 메모리 클래스
enum MemoryType {SLC, MLC, TLC, QLC}

class USBMemory extends StorageDevice {
    private MemoryType memoryType;

    public USBMemory(String modelName, int capacity, MemoryType memoryType) {
        super(modelName, capacity);
    ...
    @Override
    public String toString() {
        return super.toString() + ", Memory Type: " + memoryType;
    }
}

// 컴퓨터 클래스
class Computer {
    private StorageDevice[] sataDevices = new StorageDevice[4];
    private StorageDevice[] usbDevices = new StorageDevice[3];
    private int sataCount = 0;
    private int usbCount = 0;

    public void connectSataDevice(StorageDevice device) {
        if (sataCount < 4) {
            sataDevices[sataCount++] = device;
        } else {
            System.out.println("SATA 연결 장치가 모두 사용 중입니다.");
        }
    }

    public void connectUsbDevice(StorageDevice device) {
        if (usbCount < 3) {
            usbDevices[usbCount++] = device;
        } else {
            System.out.println("USB 연결 장치가 모두 사용 중입니다.");
        }
    }

    public void printConnectedDevices() {
        System.out.println("Connected SATA Devices:");
        for (int i = 0; i < sataCount; i++) {
            System.out.println(sataDevices[i]);
        }

        System.out.println("\nConnected USB Devices:");
        for (int i = 0; i < usbCount; i++) {
            System.out.println(usbDevices[i]);
        }
    }
}
...
public class Prac3 {
    public static void main(String[] args) {
        // SATA 연결 장치 생성
        InternalHDD internalHDD1 = new InternalHDD("SATA_HDD_1", 1000, 7200, 3.5);
        InternalHDD internalHDD2 = new InternalHDD("SATA_HDD_2", 2000, 5400, 2.5);

        // USB 연결 장치 생성
        ExternalHDD externalHDD = new ExternalHDD("USB_HDD", 500, 5400, 2.5);
        USBMemory usbMemory = new USBMemory("USB_Memory", 128, MemoryType.MLC);

        // 컴퓨터에 연결
        Computer computer = new Computer();
        computer.connectSataDevice(internalHDD1);
        computer.connectSataDevice(internalHDD2);
        computer.connectUsbDevice(externalHDD);
        computer.connectUsbDevice(usbMemory);

        // 컴퓨터에 연결된 저장 장치 출력
        computer.printConnectedDevices();
    }
}
...
public class Prac4 {
    public static void main(String[] args) {
        // Address 객체 생성
        Address address1 = new Address("John Doe", "123-456-7890", "123 Main St");
        Address address2 = new Address("Jane Smith", "987-654-3210", "456 Oak Ave");
        Address address3 = new Address("John Doe", "123-456-7890", "123 Main St");

        // hashCode 비교
        System.out.println("HashCode 1: " + address1.hashCode());
        System.out.println("HashCode 2: " + address2.hashCode());
        System.out.println("HashCode 3: " + address3.hashCode());

        // equals 비교
        System.out.println("Equals 1 and 2: " + address1.equals(address2));
        System.out.println("Equals 1 and 3: " + address1.equals(address3));
    }
}
...
class ComputerComponent {
    ...
    // 생성자
    public ComputerComponent(int itemNumber, String name, String manufacturer, double price,
                             int numInStock, int numCores, double speed, String modelNumber, String serialNumber) {
    ...
    public void displayInfo() {
        ...
        System.out.println("Stock: " + numInStock);
        System.out.println("Model Number: " + modelNumber);
        System.out.println("Serial Number: " + serialNumber);
    }
}

// CPU 클래스: 상속 ComputerComponent
class CPU extends ComputerComponent {
    // 추가 속성
    private String socketType;

    // CPU 클래스 생성자
    public CPU(int itemNumber, String name, String manufacturer, double price,
               int numInStock, int numCores, double speed, String modelNumber,
               String serialNumber, String socketType) {
        super(itemNumber, name, manufacturer, price, numInStock, numCores, speed, modelNumber, serialNumber);
...
class GPU extends ComputerComponent {
    ...
    // GPU 클래스 생성자
    public GPU(int itemNumber, String name, String manufacturer, double price,
               int numInStock, int numCores, double speed, String modelNumber,
               String serialNumber, String chipset, String memoryType,
               int memorySpeed, int memorySize) {
        super(itemNumber, name, manufacturer, price, numInStock, numCores, speed, modelNumber, serialNumber);
...
public class Prac5 {
    public static void main(String[] args) {
        // CPU 객체 생성
        CPU cpu = new CPU(1, "Intel i7", "Intel", 299.99, 10, 8, 3.6, "i7-1234", "SN123456", "LGA1200");

        // GPU 객체 생성
        GPU gpu = new GPU(2, "NVIDIA RTX 3080", "NVIDIA", 699.99, 5, 8704, 1.71, "RTX3080", "SN789012",
                "Ampere", "GDDR6X", 19, 10);
...
class Problem6 {
    public static void main(String[] args) {
        String[] cities = {"Busan", "Daegu", "Incheon", "Gwangju", "Daejeon", "Ulsan"};

        // Creating an instance of StringComparator
        StringComparator stringComparator = new StringComparator();

        // Sorting the array of cities using StringComparator
        Sorter.sort(cities, stringComparator);
...
class StringComparator implements Comparator {
    ...
    public int compare(Object o1, Object o2) {
        // Assuming that o1 and o2 are strings, you can use compareToIgnoreCase for case-insensitive sorting
...
class Rectangle {
    ...
    public void setCoordinates(double... values) {
        if (values.length % 2 != 0) {
            System.out.println("잘못된 좌표 개수입니다.");
            return;
        }

        coordinates = new ArrayList<>();
        for (int i = 0; i < values.length; i += 2) {
            coordinates.add(new Point(values[i], values[i + 1]));
        }
    }

    public double[] getCenter() {
        if (coordinates == null || coordinates.size() != 4) {
            System.out.println("올바른 좌표가 설정되지 않았습니다.");
            return null;
        }
...
public class Prac1 {
    public static void main(String[] args) {
        ...
        rectangle.setCoordinates(-100, -50, -100, 100, 100, 50, 50, -50);

        double[] center = rectangle.getCenter();

        if (center != null) {
            System.out.println("중심 좌표: (" + center[0] + ", " + center[1] + ")");
        }
    }
}
...
public class Prac2 {
    ...
    // 1부터 100 사이의 정수를 무작위로 10개 생성하여 리스트에 저장
    public void setNumbers() {
        Random random = new Random();
        for (int i = 0; i < 10; i++) {
    ...
    // 리스트의 짝수들의 평균을 계산하고 반환
    public double getAverageOfEvenNumbers() {
        int sum = 0;
        int count = 0;
        ...
        if (count == 0) {
            System.out.println("리스트에 짝수가 없습니다.");
            return 0.0;
        }
    ...
    public static void main(String[] args) {
        ...
        // 무작위로 10개의 정수 생성 및 리스트에 저장
        randomNumbers.setNumbers();

        // 짝수의 평균을 계산하고 출력
        double averageOfEvenNumbers = randomNumbers.getAverageOfEvenNumbers();
        System.out.println("리스트 요소 중에서 짝수의 평균값 = " + averageOfEvenNumbers);
    }
}
...
public class Problem03_Tour {

    private String[] cities = {"Seoul", "New York", "Sydney", "Tokyo", "Beijing"};

    // 접근 제어자를 지정하여 메서드를 완성
    ...
    // main 메서드를 완성
    public static void main(String[] args) {
        // Problem03_Tour 객체 생성
        Problem03_Tour tour = new Problem03_Tour();
...
public class Prac6 {
    public static void main(String[] args) {
        // Color 객체를 담을 ArrayList 생성
        ArrayList<Color> colorList = new ArrayList<>();
        colorList.add(new Color("Pink", "#FFC0CB"));
        colorList.add(new Color("Yellow", "#FFFF00"));
        colorList.add(new Color("Red", "#FF0000"));
        colorList.add(new Color("Cyan", "#00FFFF"));
        ...
        colorList.add(new Color("Orange", "#FFA500"));
        colorList.add(new Color("Black", "#000000"));

        // 사용자로부터 색상 이름 입력 받기
        Scanner scanner = new Scanner(System.in);
        System.out.print("색상 이름을 입력하세요: ");
        String inputColorName = scanner.nextLine();

        // 입력받은 색상 이름에 해당하는 색상 코드 찾아서 출력
        boolean found = false;
        for (Color color : colorList) {
            if (color.getColorName().equalsIgnoreCase(inputColorName)) {
                System.out.println("색상 이름: " + color.getColorName());
                System.out.println("색상 코드: " + color.getColorCode());
                found = true;
                break;
            }
        }
        ...
        if (!found) {
            System.out.println("해당하는 색상 이름을 찾을 수 없습니다.");
        }
    }
}
...
public class Prac7 {
    public static void main(String[] args) {
        ...
        // 입력받은 색상 이름에 해당하는 색상 코드 찾아서 출력
        String colorCode = colorMap.get(inputColorName);
        if (colorCode != null) {
            System.out.println("색상 이름: " + inputColorName);
            System.out.println("색상 코드: " + colorCode);
        } else {
            System.out.println("해당하는 색상 이름을 찾을 수 없습니다.");
        }
    }
...
public class Prac10 {
    public static void main(String[] args) {
        // 국제 전화 코드와 국가 이름을 매핑하는 Map 생성
        Map<Integer, String> internationalCodes = new HashMap<>();
        internationalCodes.put(82, "South Korea");
        internationalCodes.put(86, "China");
        internationalCodes.put(81, "Japan");
        internationalCodes.put(886, "Taiwan");
        internationalCodes.put(1, "United States of America");
        internationalCodes.put(49, "Germany");
        internationalCodes.put(44, "United Kingdom");
        internationalCodes.put(65, "Singapore");
        internationalCodes.put(84, "Vietnam");
        internationalCodes.put(33, "France");

        // 코드 첫 자리가 8인 국가를 출력
        System.out.println("코드 첫 자리가 8인 국가:");
        for (Map.Entry<Integer, String> entry : internationalCodes.entrySet()) {
            int code = entry.getKey();
            if (String.valueOf(code).startsWith("8")) {
                System.out.println("국제 전화 코드: " + code + ", 국가 이름: " + entry.getValue());
            }
        }
    }
}
...
public class Prac11 {
    public static void main(String[] args) {
        ...
        // 문자, 숫자, 기타 문자(특수 기호와 공백 문자) 개수 계산
        int digitCount = 0;
        int letterCount = 0;
        int otherCount = 0;
        ...
        // 결과 출력
        System.out.println("DIGITS=" + digitCount + ", LETTERS=" + letterCount + ", OTHERS=" + otherCount);
...
- 입출력 관련된 모든 코드를 한 개 try{} 블럭 안에 넣고, 예외는 Exception만 처리한다.

1. 다음 문장은 미국 소설가 마크 트웨인이 쓴 "허클베리핀의 모험"에 나오는 글이다. 이 문장을 코드에서 문자열로 저장하고 이 문자열을 "finn.txt" 파일에 저장하는 프로그램을 작성한다.
```
...
2. 1번 문제에서 생성한 "finn.txt" 파일 내용을 읽고 단어들을 모두 소문자 형태로 만든다. 그리고 "I", "a", "an", "the", "we" 단어를 제외하고 빈도가 2번 이상 나타나는 단어를 "단어 = 횟수" 형태로 화면에 출력하는 프로그램을 작성한다. 단어를 분리하는 것은 공백 문자와 특수 기호가 있으면 단어를 구분하는 것으로 본다.
```java

```

3. 학생 한 명의 성적이 "국어, 수학, 영어, 과학" 형태(예: 80, 90, 87, 100)로 "score.txt" 파일에 저장되어 있다. 과목별 성적 정보를 담을 수 있도록 StudentScore라는 클래스를 구현하고, 파일에 있는 데이터를 읽어서 화면에 출력하는 프로그램을 작성한다. StudentScore에 필요한 메소드가 있다면 자유롭게 추가한다.
```java

```

4. 학생 5명의 성적이 다음 형태로 "scores.txt" 파일에 저장되어 있다.
```
홍길동, 90. 80, 70, 95
백연우, 89, 87, 85, 94
김예은, 85, 89, 94, 70
...
강채영, 90, 87, 78, 98
```
앞에서 만들었던 StudentScore 클래스를 상속 받는 새로운 클래스를 만들고 학생 이름 정보를 포함시킨다. 새로운 클래스에 필요한 멤버 함수나 메소드가 있다면 자유롭게 추가한다. 파일에서 정보들을 읽어서 StudentScore 객체를 담을 수 있는 리스트에 저장하고, 학생 이름과 과목별 점수를 출력한다. 그리고 과목별 평균 점수를 계산해서 출력한다.
```java

```

5. 1번 문제에서 저장한 "finn.txt"를 읽고 영문 모음('a', 'e', 'i', 'o', 'u')이 두 번 이상 연속으로 나오는 단어들을 찾아서 한 단어씩 "vowels.txt"에 저장한다. 영문 모음이 두 번 이상 나오는 단어의 예는 "book", "found" 등이다.
```java

```

6. 문법상 오류가 없는 자바 코드 파일을 읽고 "//"로 시작되는 주석문을 제거한 코드를 다른 파일에 저장하는 프로그램을 작성한다. 자바 코드 파일 이름은 사용자로부터 입력 받고, 새로 생성하는 파일은 기존 파일 이름과 확장자 사이에 "new."를 붙인다.
```java

```

7. 문법상 오류가 없는 자바 코드 파일을 읽고 "/.../" 형태의 주석문을 제거한 코드를 다른 파일에 저장하는 프로그램을 작성한다. 자바 코드 파일 이름은 사용자로부터 입력 받고, 새로 생성하는 파일은 기존 파일 이름과 확장자 사이에 "new."를 붙인다.
```java

```

8. 두 개 텍스트 파일을 붙여주는 프로그램을 작성한다. 두 개 파일 이름과 새로 생성되는 파일의 이름은 사용자로부터 입력 받는다. 이 때 두 개 원본 파일 이름과 새로 생성될 파일의 이름은 순서대로 명령행 인자로 입력 받는다. 다음은 실행 예시를 보인다.
    - java Problem08 fst.txt secnd.txt newfile.txt
```java

```

9. 텍스트 파일을 읽고 사용자로부터 주어진 단어를 검색하는 프로그램을 작성한다. 텍스트 파일 이름과 검색할 단어는 순서대로 명령행 인자로 전달 받는다. 한 줄 단위로 단어를 검색하되 대문자와 소문자를 구별하고, 단어가 다른 단어 중간에 있더라고 찾는 것으로 처리한다. 검색하는 단어가 있는 문장을 "줄 번호: 문자" 형태로 화면에 출력한다. 이 때 줄 번호는 1부터 시작한다.
```java

```

10. 텍스트 파일을 읽고 각 단어의 빈도수를 찾아서 화면에 출력하는 프로그램을 작성한다. 텍스트 파일의 이름은 사용자로부터 Scanner 클래스를 이용해서 입력 받는다.
```java

```

11. 명령행 인자로 주어진 텍스트 파일을 읽고 전체 단어의 개수와 특수 문자 개수를 출력하는 프로그램을 작성한다.
```java

```

12. 명령행 인자로부터 파일 이름을 입력 받고, 파일을 읽어서 각 행에 있는 단어 개수를 세서 다른 파일에 순차적으로 저장하는 프로그램을 작성한다. 단어를 구별하는 것은 공백 문자와 특수 기호로 분리한다. 새로 저장하는 파일의 이름은 원본 파일의 확장자 앞에 "data."를 추가한다.
```java

```

13. 오름차순으로 정렬된 정수들이 저장되어 있는 두 개 파일을 읽으면서 정렬하면서 새로운 파일에 바로 저장하는 프로그램을 작성한다. 데이터를 읽으면서 바로 정렬해서 다른 파일에 저장하기 때문에 자료 구조를 사용하지 않는다. 이미 정렬되어 있는 파일에 있는 데이터의 개수는 정해진 것이 아니고, 파일의 끝에 도달할 때까지 읽어서 처리한다. 또 두 개 파일의 정수 개수는 동일하지 않을 수 있다. 이미 정렬되어 있는 두 개 파일과 새로 정렬된 데이터가 저장될 세 번째 파일 이름은 모두 명령행 인자를 통해서 전달 받는다.
```java

```

14. 정렬되어 있지 않는 정수들이 포함된 두 개 파일의 데이터를 각각 자료 구조(리스트 또는 배열)에 저장하고, 이들을 합쳐서 새로운 자료 구조에 넣은 후 정렬해서 파일에 저장하는 프로그램을 작성한다. 각 정수 파일에 저장되어 있는 숫자 개수는 정해지지 않았으며 파일의 끝에 도달할 때까지 처리한다. 자료 구조에 저장 되어 있는 데이터를 정렬하는 것이므로 자바에서 제공하는 정렬 기능을 사용해도 된다. 파일 이름은 명령행 인자로 전달 받는다.
```java

```

15. 다음 내용이 "config.ini"에 저장되어 있다고 가정한다. 이 파일은 Key="Value" 형태로 구성된다. 파일 내용을 모두 읽어서 맵에 저장하고, 사용자로부터 Key에 해당되는 문자열을 입력 받아 값(따옴표를 제외한 문자열)을 출력하는 프로그램을 작성한다. 사용자로부터 문자열을 입력받는 것은 Scanner 클래스를 사용한다.
```
APPLICATION="setup.ini"
APPLICATION CLIENT="client"
CAMERA="camera3"
DISPLAY="display 1"
DISPLAY MONO="display2"
DISPLAY IIS="web display"
```
```java
...
- 오류와 예외
    ...
    - 예외
        - 런타임 오류
        - 프로그램 실행 중에 발생하면 자바 프로그램을 비정상적으로 종료시킴
...
- 예외의 구분
    ...
    |런타임 예외(RuntimeException)|프로그래밍을 통해 막을 수 있는 오류, 미확인 예외라고도 함. 컴파일러가 예외 처리 코드 유무를 확인하지 않음|IndexOutOfBoundsException|
    |기타 예외(RuntimeException을 제외한 나머지 Exception)|프로그래밍으로 예방할 수 없는 예외, 확인 예외라고도 함. 컴파일러가 예외 처리 코드 유무를 확인함|FileNotFoundException|

- 런타임 예외
    - 컴파일러가 확인하지 않는 예외
    - 예외 처리 코드가 없어도 강요하지 않음

    |예외|설명|
    |:---|:---|
    |ArithmeticException|연산 예외|
    |IndexOutOfBoundsException|배열 인덱스 범위 초과 예외|
    |NullPointerException|객체 참조 예외|
    |ClassCastException|클래스 형변환 예외|

    - 산술 연산 예외
        - 대표적으로 숫자를 0으로 나눌 때 발생
    - 배열 인덱스 범위 초과 예외
        - 배열 요소에 접근하면서 인덱스 번호가 배열 크기를 벗어나면 발생
    - 클래스 형변환 예외
        - 객체를 다른 클래스 종류로 변환해서 저장하거나 사용할 때 발생

- 기타 예외
    - 컴파일러가 예외 처리 코드를 확인하는 예외
    - 대표적으로 파일 입출력 예외들이 해당
    - 예외 처리 코드가 없으면 컴파일 오류 발생

### 예외 클래스

- 자바의 예외는 클래스로 구현되어 있음

- 자바의 에러나 예외 클래스들은 Throwable의 자손
- Throwable에는 printStackTrace() 함수가 있음
- 예외 클래스들은 다수의 패키지에 분산되어 있음
    - 각각 다른 패키지들을 import 해야 함

### 예외 처리

- 예외 처리 방법
    - 예외를 위임
    - 특정 예외가 발생했을 때 다른 예외로 바꿔서 전달
    - 직접 처리 코드를 실행
...
- 예외를 위임
    ...
    - 예외를 위임시키는 코드
    ```
    void printElement(int[] arr, int index) throws 
        ArrayIndexOutOfBoundsException {
        System.out.print(arr[index]); 
    }
    ```
    - 함수 내부에서 여러 개 예외 발생 시 콤마로 분리

- 예외를 변환해서 전달
    - 예외를 위임시키는데 다른 예외로 바꿔서 전달
    ```
    public void printElement2(int[] arr, int index) throws IllegalArgumentException {
        try {
            System.out.print(arr[index]); 
        }
...
- 예외 처리 코드 작성
    - 예외 처리 코드는 "try", "catch", "finally" 같은 키워드를 이용해서 코드 블록을 지정
    - try 블록에는 반드시 한 개 이상의 catch 코드 블록이 있어야 함
        - 실행 중에 예외가 발생할 수 있는 코드 삽입
        - 예외가 발생하지 않는 코드와 함께 넣어도 됨
    - catch 블록
        - 어떤 예외를 처리할 것인지 지정(예외 클래스를 명시)
        - 예외 발생 시 처리할 코드 삽입
        - try 블록에서 여러 가지 예외가 발생할 수 있다면, 각각에 대응하는 catch 블록 사용 가능
            - 또는 부모 예외 클래스를 사용해서 일괄적인 처리 가능
    - finally 블록
        - 예외의 발생 여부와 관계없이 무조건 실행시켜야 하는 코드 삽입
        - 생략 가능
    ```
    try {
        // 예외가 발생할 수 있는 코드
        ...
        // 예외클래스에 해당되는 예외가 발생했을 때 실행될 코드
    }
    finally { // 생략 가능
        // 예외 발생 여부와 관계없이 항상 실행되어야 하는 코드
    }
    ```
    ...
    - finally 블록 사용 예제
        ...
        |예외 종류|설명|
        |:---|:---|
        |InputMismatchException|다음에 읽을 데이터가 정수가 아니거나 정수형의 범위를 벗어날 때 발생|
        |NoSuchElementException|더 이상 읽을 수 있는 데이터가 없을 때 발생|
        |IllegalStateException|Scanner가 닫혔을 때(close() 함수가 실행된 후에 읽을 때) 발생|

    - 한 개의 catch 블록에서 여러 개 예외 처리
        - 자바 7부터 catch 블록 한 개에서 여러 개 예외 처리 가능
        ```
        try{
            //예외가 발생할 수 있는 코드
        }
        catch(예외클래스1 | 예외클래스2 | 예외클래스3 변수_이름){
            //예외클래스1, 예외클래스2, 예외클래스3
            //예외가 발생했을 때 실행될 코드
        }
        finally{ //생략 가능
            //예외 발생 여부와 관계없이 항상 실행되어야 하는 코드
    ...
    - 예외 처리와 자동 리소스 닫기
        - 자바 7부터 예외 처리 코드에서 사용되는 리소스를 자동으로 닫아주는 기능 추가
        - Scanner를 사용할 때 close()를 부르는 것과 같은 경우
        ```
        try(자동으로 닫을 리소스 객체 생성 후 참조 변수에 저장){
            //리소스 객체 참조 변수 사용 코드
        }
        catch(예외 변수){
            //예외가 발생했을 때 실행할 코드
        ...
        - 리소스를 여러 개 사용하면 세미콜론으로 분리
        - 리소스 객체가 java.lang.AutoCloseable 인터페이스를 구현해야 함

### 자신만의 예외 만들어보기

- 기존의 예외 클래스들이 부적합하면 본인만의 예외 클래스를 만들어서 사용 가능
- 새로 만들어지는 예외는 Exception 클래스에서 상속 받도록 함
- 새로운 예외를 발생시키는 것은 다른 예외로 변환해서 발생시켰던 것과 비슷함

### 연습 문제

1. 파일의 내용을 읽어서 다른 파일로 복사하는 프로그램을 작성한다. 원본 파일의 이름은 "finn.txt"(8장 연습 문제 1번 참고)이고 새로 만들어질 파일의 이름은 "finn2.txt"로 한다. finn.txt파일은 자바 코드와 동일한 폴더에 존재한다고 가정하고, BufferedReader를 이용해서 파일을 읽어서 새로운 파일에 저장하는 형태로 파일을 복사하는 프로그램을 작성한다. 원본 파일이 존재하지 않는 경우와 입출력 오류에 대비하는 예외처리 코드를 작성한다.
```java

```

2. 다음 코드는 배열에 있는 점수의 평균을 구하는 프로그램이다. 배열의 범위를 넘피는 부분까지 합을 구하고 있어 예외가 발생한다. 이 프로그램을 수정해서 예외 처리를 통해 배열의 범위가 넘치면 "배열의 범위를 넘어갔습니다"만 화면에 출력하고, 합계를 구할 때는 무시하도록 해서 제대로 평균을 구하는 프로그램으로 수정한다.
```java
class Problem02{
    public static void main(String[] args){
...
3. 2번에서 만든 문제를 배열 대신 ArrayList를 사용하고, 마찬가지로 리스트의 범위를 넘어서 예외를 발생시키는 프로그램을 작성해본다. 그리고 이 프로그램을 수정해서 예외 처리 코드를 넣은 후, 리스트의 범위를 넘어가면 "리스트의 범위를 넘어갔습니다"만 화면에 출력하고, 합계를 구할 때는 무시하는 프로그램을 작성한다.
```java

```

4. "Shapes.txt"에 다음에 보인 것처럼 도형 이름과 좌표 정보가 있다. Shapes.txt 파일을 읽고 객체를 생성한 후에 면적을 계산해서 화면에 출력하는 프로그램을 작성한다. 단 여기서는 파일에 읽는 데이터가 부정확한 경우도 있다. 예를 들어 삼각형을 구성하는 좌표가 6개 필요한데, 이 파일에는 5개 밖에 없다. 이러한 경우에, "InvalidShapeData" 예외를 발생시키고 해당 도형에 대한 정보는 처리하지 않고 무시하는 것으로 프로그램을 작성한다. 즉 다음 데이터에서는 삼각형 데이터는 무시되고, 사각형과 원에 대한 면적 정보만 화면에 출력된다.
```
4
Rectangle
250.0 250.0 400.0 400.0
Triangle
50.0 56.6987 25.0 100.0 75.0
Rectangle
350 350 450 450
Circle
150.0 150.0 250
...
5. 사용자로부터 문자열을 입력 받고 정수로 변환해서 반환하는 함수를 작성한다. 정수가 아닌 문자열을 변환하는 경우 NumberFormatException이 발생한다. 이를 처리해서 "정수로 변환할 수 없는 문자열이 입력되었습니다"를 화면에 출력하고 0을 반환하는 함수를 구현한다. 그리고 이 함수를 이용해서 예외를 발생시키는 경우와 발생시키지 않는 경우에 대한 main() 함수를 작성한다.
```java

```

6. 5번 문제에서 작성한 함수를 수정해서 NumberFormatException이 발생하면 이 함수를 호출한 코드에 예외를 위임시키는 프로그램을 작성한다.
```java

```

7. 숫자값을 0으로 나누려고 하면 ArithmaticException이 발생한다. 사용자로부터 두 개 정수를 5회 입력 받아 처음 입력 받는 값을 두 번째로 입력 받은 값으로 나눈 결과를 출력하는 프로그램을 작성한다. 단 사용자가 두 번째 정수로 0을 입력하면 예외가 발생하는데 예외 처리 코드를 추가해서 ArithmaticException이 발생하면 "0으로 나눌 수 없습니다"라고 출력하고 다시 입력을 받는 프로그램을 작성한다.
```java

```

8. ClassCastException이 발생할 수 있는 코드를 작성해본다. 그리고 이 프로그램에 예외 처리 코드를 추가 해서 예외가 발생할 때 바로 프로그램이 종료되는 것이 아니라 "클래스 형 변환에 문제가 있습니다"를 출력하는 프로그램을 작성한다.
```java
